...
class Loss_3DCellSeg(torch.nn.Module):
    """ 
    Loss from 3D Cell Seg 

    https://github.com/AntonotnaWang/3DCellSeg/blob/main/func/loss_func.py
    ...
    def forward(self, output, targets):
        ...
        seg_output_f = probs[:,1,:,:,:]
        seg_output_bb = torch.cat((probs[:,0,:,:,:], probs[:,2,:,:,:]), dim=1)

        seg_groundtruth_bb = probs[:,1,:,:,:]
        seg_groundtruth_f = torch.cat((probs[:,0,:,:,:], probs[:,2,:,:,:]), dim=1)
        
        loss = dice_loss_org_weights(seg_output_bb, seg_groundtruth_bb, weights_bb) 
        loss += dice_loss_II_weights(seg_output_f, seg_groundtruth_f, weights_f)
