...
def perlin_optim_3d(dims, grid_points, octaves=1, sf=2, order=1):
    '''
    Optimizes perlin noise generation. Generates down-sampled
    version then upscales by sf to get output with dims.
    '''

    assert len(dims) == len(grid_points)
    
    xres, yres, zres = dims

    pn = generate_fractal_noise_3d([xres//sf, yres//sf, zres//sf], grid_points, octaves=octaves)

    out = zoom(pn, (sf,sf,sf), order=order)
