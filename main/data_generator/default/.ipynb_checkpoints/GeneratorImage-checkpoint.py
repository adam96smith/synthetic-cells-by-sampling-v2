...
parser.add_argument('--mask-dir', type=str, required=True, help='Path to 3D Masks')
parser.add_argument('--sampler-dir', type=str, required=True, help='Directory with Sampled Fluorescence')
parser.add_argument('--all-samplers',action='store_true', help='Use all Samplers')
parser.add_argument('--one-label',action='store_true', help='Set all mask labels to 1')
parser.add_argument('--resample',  action='store_true',  help='Activate to resample original fluorescence to ground truth mask')
parser.add_argument('--sub-folder', type=str, default='custom_texture/', help='Target Directory for textured Images')
parser.add_argument('--config', type=str, required=True, help='Config. File for Synthetic Images')
...
speckle_dI = config['AUGMENTATION']['SPECKLE_dI']
speckle_p = config['AUGMENTATION']['SPECKLE_p']
speckle_beta = config['AUGMENTATION']['SPECKLE_beta']
...
for i in tqdm(range(N), desc='Generating Image Textures'):
    ...
    # load data
    mask_labels = tifffile.imread(mask_file).astype(int)

    if args.one_label:
        mask_labels = (mask_labels>0).astype(int)

    # Calculate the new full distance map
    dist_map, indices = distance_transform_edt(mask_labels==0, sampling=sampling, return_indices=True)
    for j in range(mask_labels.max()):
        dist_map += -distance_transform_edt(mask_labels==j+1, sampling=sampling)
    partitions = mask_labels[tuple(indices)].astype(np.uint8)

    if np.random.rand() < warp_dist_p:

        grid_points = np.random.choice([2**i for i in range(2,5) if 2**i < np.min(mask_labels.shape)/2])
        pN_1 = perlin_optim_3d(npt_seq(mask_labels.shape), [grid_points,grid_points,grid_points], octaves=1, sf=2, order=1)
        pN_1 = np.exp( warp_dist_A*pN_norm( pN_1[:mask_labels.shape[0],:mask_labels.shape[1],:mask_labels.shape[2]]) )
    ...
    image = texture_mask((mask_labels>0), 
                         sampler, 
                         dist_map=dist_map, 
                         labelled_mask=mask_labels,
    ...
    tifffile.imwrite(f'{args.mask_dir}{args.sub_folder}image_{str(i+1).zfill(5)}.tif', image.astype(np.float32))
